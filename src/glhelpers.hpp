...
namespace DGL {
    ...
    class Texture {
        ...
        inline void bind(GLuint texture_unit=GL_TEXTURE0,bool cubemap=false) {
            if(!id) {  err_printf("trying to bind a null texture\n");   }
            glActiveTexture(GL_TEXTURE0+texture_unit);
            if(cubemap) glBindTexture(GL_TEXTURE_CUBE_MAP, id);
        ...
        inline void configure(bool smoth=false,bool mipmap=false, bool clamp=false) {
            ...
            glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MIN_FILTER, smoth ? GL_LINEAR  : GL_NEAREST);
   ...
   class CPUTextureBuffer {
        ...
        inline Texture send_to_gpu(bool smoth=false,bool mipmap=false, bool clamp=false) { 
            log_printf("Sending CPUTextureBuffer to gpu...\n");
            GLuint channels_format=GL_LUMINANCE;
            if(channels==3) channels_format=GL_RGB;
            if(channels==4) channels_format=GL_RGBA;

            Texture texture;
            texture.create();
            texture.bind();
            texture.set_image(buffer,width,height,channels_format,GL_UNSIGNED_BYTE);
